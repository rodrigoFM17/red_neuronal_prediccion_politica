...
def cross_validate_neural_network(X_text, X_struct, y, n_folds=5, epochs=30, batch_size=16):
    """
    Realiza validación cruzada para la red neuronal
    """
    # Configurar KFold
    ...
    results = {
        ...
        'best_model': None,
        'best_score': 0.0,
        'best_fold': 0  # Añadir best_fold con valor inicial
    }
    ...
    for train_index, test_index in kf.split(X_text):
        ...
        if prec > results['best_score']:
            ...
            results['best_fold'] = fold_no  # Guardar el número del mejor fold
...
def train_final_model(X_text, X_struct, y, cv_results, epochs=30, batch_size=16):
    """
    Entrena el modelo final utilizando información del mejor fold
    """
    # Dimensiones del modelo
    input_text_dim = X_text.shape[1]
    input_struct_dim = X_struct.shape[1]
    ...
    if 'best_model' in cv_results and cv_results['best_model'] is not None:
        ...
        # Opcional: Refinamiento adicional con todos los datos
        if input_struct_dim > 0:
            X_combined = [X_text, X_struct]
        else:
            X_combined = X_text
            
        # Fine-tuning con todos los datos (opcional)
        final_model.fit(
            X_combined, y,
    ...
    # Opción 2: Crear nuevo modelo y entrenarlo con todos los datos
    else:
        ...
        # Configurar early stopping con una pequeña porción para validación
        early_stopping = EarlyStopping(
            monitor='val_loss',
            patience=5,
            restore_best_weights=True
